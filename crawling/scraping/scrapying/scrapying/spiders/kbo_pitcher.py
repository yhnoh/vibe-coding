...
class KboPitcherSpider(scrapy.Spider):
    """KBO 투수 목록을 수집하는 스파이더.

    실행 방법:
        scrapy crawl kbo_pitcher                          # 전체 투수 목록
        scrapy crawl kbo_pitcher -a season=2025           # 특정 시즌
        scrapy crawl kbo_pitcher -a team=HT               # 특정 팀만
        scrapy crawl kbo_pitcher -a all_teams=true        # 팀별로 각각 수집

    동작 흐름:
        start() → /statistics/.../players?playerType=PITCHER API 호출
              ↓
    ...
    async def start(self):
        base_url = (
            ...
            f"?sortField=pitcherEra&sortDirection=asc&playerType=PITCHER&pageSize=500"
        )
    ...
    def parse(self, response, team_code):
        ...
        self.logger.info(f"\n=== 투수 목록 [{label}]: {response.url} ===")

        if "json" in content_type:
            item = CrawledItem()
            item["source"] = "naver-sports"
            item["data_type"] = f"kbo-pitcher-{team_code}" if team_code else "kbo-pitcher"
